...
export function RetroWindow({ title, onClose, children, className = "", noPadding = false, headerActions, onTitleClick, confirmOnClose = false, hasUnsavedChanges = false, onSaveBeforeClose, sfx }) {
  ...
  const handleCloseClick = () => {
    if (!onClose) return;
    if (confirmOnClose) {
      if (typeof hasUnsavedChanges === 'function' ? hasUnsavedChanges() : hasUnsavedChanges) { setConfirmType('unsaved'); return setShowConfirm(true); }
      setConfirmType('simple'); return setShowConfirm(true);
    }
    playAudio('click', sfx);
    onClose();
  };
    ...
    <>
      ...
      <div className="retro-bg-accent retro-border border-t-0 border-l-0 border-r-0 border-b-2 flex justify-between items-center p-2 flex-shrink-0">
        <div className={`flex gap-2 items-center text-[var(--text-main)] ${onTitleClick ? 'cursor-pointer hover:opacity-70 transition-opacity' : ''}`} onClick={onTitleClick}>
          ...
          <span className="font-bold lowercase text-xs sm:text-sm flex items-center gap-2 truncate max-w-[150px] sm:max-w-none">{title}</span>
        </div>
      ...
      <div className={`flex-1 overflow-y-auto flex flex-col text-[var(--text-main)] ${noPadding ? '' : 'p-3 sm:p-4'}`}>{children}</div>
    </div>
      {showConfirm && (
        <ConfirmDialog
          ...
          showCancel={false}
          onSave={() => { playAudio('click', sfx); if (onSaveBeforeClose) onSaveBeforeClose(); setShowConfirm(false); onClose && onClose(); }}
