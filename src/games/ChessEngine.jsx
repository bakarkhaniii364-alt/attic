...
export function ChessEngine({ config, setScores, onBack, sfx, onWin, onShareToChat, onSaveToScrapbook, profile, userId }) {
  ...
  if (gameOverResult) {
      return ( <ShareOutcomeOverlay partnerNickname={(typeof config !== "undefined" && config?.mode === "vs_ai") || (typeof mode !== "undefined" && mode === "vs_ai") || (typeof gameMode !== "undefined" && gameMode === "vs_ai") ? "AI" : undefined} gameName="Chess" stats={{ Result: gameOverResult, "Moves Played": history.length, "Mode": config.mode==='vs_ai'?'VS AI':'Local 1v1' }} onClose={() => { loadFen(INITIAL_FEN); setGameOverResult(null); onBack();}} onShareToChat={onShareToChat} onSaveToScrapbook={onSaveToScrapbook} sfx={sfx} /> );
  }
