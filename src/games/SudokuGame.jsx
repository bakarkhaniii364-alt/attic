            ...
            {board.map((row, r) => row.map((cell, c) => {
              ...
              return (
                  <div key={`${r}-${c}`} 
                    ...
                    style={cellStyle}
                    className={`relative flex items-center justify-center text-sm sm:text-xl md:text-2xl cursor-pointer ${borders} ${extraClass} ${cell.fixed ? 'font-bold text-[var(--border)]' : 'text-[var(--primary)]'} transition-colors`}
                  >
                      {cell.val || ''}
