...
function LivingBackground({ weather }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // Add single lightning instance for stormy/thunderous weather
      if (weather === 'thunder' || weather === 'storm') {
         newElements.push({ id: 'flash', type: 'lightning', delay: 0, duration: 4 + Math.random() * 3 });
      }
  ...
  return (
    <div className="weather-layer">
      {elements.map(e => {
        ...
        if (e.type === 'lightning') return <div key={e.id} className="lightning-flash" style={{ animationDuration: `${e.duration}s` }} />
        return null;
      })}
    </div>
  );
