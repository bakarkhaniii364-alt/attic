...
const PixelPet = React.memo(({ happy, sleeping, onClick, skin }) => {
  ...
  let frames = 4; let yOffset = 0; 
  if (isAltSkin) {
    if (sleeping) { yOffset = 31; frames = 4; } 
    else if (isPressing) { yOffset = 21; frames = 8; } 
    else if (isHovering) { yOffset = 18; frames = 8; } 
    else if (happy > 70) { yOffset = 8; frames = 8; } 
    else { yOffset = 0; frames = 4; }
  } else {
    if (sleeping) { yOffset = 4; frames = 4; } 
    else if (isPressing) { yOffset = 7; frames = 8; } 
    else if (isHovering) { yOffset = 5; frames = 8; } 
    else if (happy > 70) { yOffset = 5; frames = 8; } 
    else { yOffset = 0; frames = 4; }
  }

  return (
    <div className={`relative cursor-pointer select-none transition-opacity ${sleeping ? 'opacity-80' : ''}`} 
      onMouseEnter={() => !sleeping && setIsHovering(true)} onMouseLeave={() => { setIsHovering(false); setIsPressing(false); }}
      onMouseDown={() => !sleeping && setIsPressing(true)} onMouseUp={() => { setIsPressing(false); if(onClick) onClick(); }}
      onTouchStart={() => !sleeping && setIsPressing(true)} onTouchEnd={() => { setIsPressing(false); if(onClick) onClick(); }}
      title={sleeping ? "Shh, pet is sleeping" : "Click to pet!"}>
      <div className="cat-sprite" style={{ backgroundImage: `url('${bgImage}')`, backgroundSize: `${bgSizeX}px ${bgSizeY}px`, backgroundPositionY: `-${yOffset * 128}px`, animation: `cat-step-${frames} ${frames * 0.15}s steps(${frames}) infinite` }} />
      ...
      {isPressing && !sleeping && ( <svg width="24" height="24" viewBox="0 0 16 16" className="absolute -top-4 right-0 animate-bounce drop-shadow-md"><path d="M4,2 L7,2 L7,3 L9,3 L9,2 L12,2 L12,3 L14,3 L14,6 L13,6 L13,8 L12,8 L12,10 L11,10 L11,11 L10,11 L10,12 L9,12 L9,13 L7,13 L7,12 L6,12 L6,11 L5,11 L5,10 L4,10 L4,8 L3,8 L3,6 L2,6 L2,3 Z" fill="#ff4d4d"/></svg> )}
    </div>
  );
});
...
function AnniversaryTimer({ anniversary }) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const start = new Date(anniversary); const now = new Date(); let diff = now - start; if (diff < 0) diff = 0;
      const totalDays = Math.floor(diff / (1000 * 60 * 60 * 24)); const years = Math.floor(totalDays / 365);
      const months = Math.floor((totalDays % 365) / 30); const days = totalDays % 30;
  ...
  if (!anniversary) return <div className="text-center text-xs opacity-50 font-bold py-2">Set your anniversary date in Settings to start the counter!</div>;
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="flex flex-wrap justify-center gap-1">
        ...
        <Unit val={elapsed.days} label="d" /> <Unit val={elapsed.hours} label="h" />
        <Unit val={elapsed.minutes} label="m" /> <Unit val={elapsed.seconds} label="s" />
      </div>
...
export const Unit = React.memo(({ val, label }) => {
  const [isFlipping, setIsFlipping] = useState(false); const [displayVal, setDisplayVal] = useState(val);
  useEffect(() => {
    if (val !== displayVal) { setIsFlipping(true); const timer = setTimeout(() => { setDisplayVal(val); setIsFlipping(false); }, 300); return () => clearTimeout(timer); }
  }, [val, displayVal]);

  return (
    <div className="flex flex-col items-center">
      <div className="relative w-10 h-10 perspective-1000">
        <div className={`w-full h-full bg-[var(--bg-main)] retro-border flex items-center justify-center font-bold text-lg text-[var(--primary)] tabular-nums transition-transform duration-300 preserve-3d ${isFlipping ? 'rotate-x-90 opacity-50' : 'rotate-x-0 opacity-100'}`}>{String(displayVal).padStart(2, '0')}</div>
...
export function CalendarReminder() {
  const [events] = useLocalStorage('calendar_events', []); const now = new Date();
  const upcoming = (events || []).filter(e => new Date(e.date) >= now).sort((a, b) => new Date(a.date) - new Date(b.date));
  const next = upcoming[0];
  if (!next) return <div className="border-t border-dashed border-[var(--border)] pt-2 mt-2 text-[10px] font-bold opacity-40 uppercase tracking-widest text-center">No upcoming events</div>;
  ...
  return (
    <div className="border-t border-dashed border-[var(--border)] pt-2 mt-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 min-w-0"><p className="font-bold text-xs truncate">{next.title || next.text || 'Event'}</p><p className="text-[10px] opacity-50">{new Date(next.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</p></div>
      </div>
    </div>
  );
}
...
export function Dashboard({ setView, profile, myDisplayName, partnerProfile, scores, doodles, onOpenDoodle, sfx, setTriggerShake, radioState, setRadioState, userId, partnerId, theme, setTheme, setProfile, sfxEnabled, setSfxEnabled, onLogout, onDelete, weather, setWeather, coupleData, setCoupleData, chatHistory, onlineUsers = {} }) {
  ...
  const unreadDoodles = safeDoodles.filter(d => d.owner_id === partnerId && !d.isRead); // DB update: owner_id instead of sender
  const [petCooldown, setPetCooldown] = useState(false);
  
  const handlePetAction = (val, msg) => {
    if (petCooldown || isSleeping) return; playAudio('click', sfx);
  ...
  return (
    <div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 relative z-10 pb-8">
                ...
                <div className="flex items-center gap-4 mt-3 bg-black/5 p-2 retro-border border-dashed">
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            {partnerProfile?.pfp ? <img src={partnerProfile.pfp} alt="partner" className="w-10 h-10 retro-border object-cover bg-white shadow-sm" /> : <div className="w-10 h-10 retro-bg-secondary retro-border flex items-center justify-center text-lg">{partnerProfile?.emoji || '👤'}</div>}
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-1.5">
                                <p className="text-sm font-bold truncate max-w-[120px] leading-none">{partnerName}</p>
                                <span className={`text-[9px] font-black uppercase px-1.5 py-0.5 retro-border leading-none ${onlineUsers[partnerId] ? 'bg-blue-400 text-white' : 'bg-gray-200 opacity-50'}`}>{onlineUsers[partnerId] === 'active' ? 'online' : onlineUsers[partnerId] === 'idle' ? 'away' : 'offline'}</span>
                            </div>
                        </div>
                    </div>
      ...
      <RetroWindow title={`${petName}.tamagotchi`} className="md:col-span-4 h-auto min-h-[12rem]">
        <div className="flex flex-col items-center text-center h-full justify-between">
          <PixelPet skin={petSkin} happy={petHappy} sleeping={isSleeping} onClick={() => handlePetAction(10)} />
