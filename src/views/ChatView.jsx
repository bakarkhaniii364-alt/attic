...
import { Phone, Video, Search, Image as ImageIcon, ChevronLeft, ChevronRight, Heart, Download, X, Reply, Smile, Edit2, Trash2, Ban, MoreVertical, Paperclip, Mic, Send, Play, Pause, Check, Pin } from 'lucide-react';
...
export function ChatView({ onClose, profile, sfx, chatHistory, setChatHistory }) {
  ...
  const startRecording = async () => {
    try {
      ...
      mediaRecorder.onstop = () => {
        ...
        if (durationSecs >= 1) { playAudio('send', sfx); const mins = Math.floor(durationSecs / 60); const secs = durationSecs % 60; const durationStr = `${mins}:${secs.toString().padStart(2, '0')}`; setChatHistory(prev => [...prev, { id: Date.now(), sender: 'me', type: 'voice', duration: durationStr, audioUrl: audioUrl, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), replyTo: replyingTo, status: 'sent' }]); }
        setReplyingTo(null); stream.getTracks().forEach(track => track.stop()); 
  ...
  const headerActions = (<div className="flex gap-2"><button onClick={() => {playAudio('click', sfx); setCalling('audio')}} className="p-1 hover:bg-black/10 rounded-md transition-colors" title="Voice Call"><Phone size={18}/></button><button onClick={() => {playAudio('click', sfx); setCalling('video')}} className="p-1 hover:bg-black/10 rounded-md transition-colors" title="Video Call"><Video size={18}/></button></div>);
    ...
    <>
      ...
      <RetroWindow title="chat_room.exe" onClose={onClose} headerActions={headerActions} onTitleClick={() => {playAudio('click', sfx); setShowDetails(!showDetails)}} className="w-full max-w-4xl h-[calc(100dvh-4rem)] max-h-[800px] flex flex-col transition-all duration-300" noPadding>
        <div className="flex flex-1 h-full overflow-hidden relative">
          {calling && (
            <div className="absolute inset-0 z-20 retro-bg-main flex flex-col items-center justify-center gap-6 animate-in fade-in"><div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full retro-bg-accent retro-border flex items-center justify-center retro-shadow-dark animate-pulse">{calling === 'video' ? <Video size={48} /> : <Phone size={48} />}</div><div className="text-center"><h2 className="text-xl sm:text-2xl font-bold">Calling Partner...</h2><p className="opacity-70 mt-2 text-sm sm:text-base">establishing P2P connection</p></div><RetroButton onClick={() => {playAudio('click', sfx); setCalling(null)}} className="bg-red-400 text-white px-8">End Call</RetroButton></div>
          )}
          ...
          <div className={`flex flex-col h-full transition-all duration-300 ${showDetails ? 'hidden md:flex md:w-2/3 border-r-2 retro-border' : 'w-full'}`}>
            ...
            <div className="flex flex-col retro-bg-accent retro-border-t">
              ...
              <form onSubmit={handleSend} className="flex gap-2 items-center p-2 sm:p-3 relative">
                ...
                <button type="button" onClick={() => {playAudio('click', sfx); fileInputRef.current?.click()}} className="p-2 retro-bg-window retro-border hover:bg-gray-100 transition-colors" title="Attach Media"><Paperclip size={18} /></button>
                <div className="flex-1 relative flex items-center">
                  <input type="text" value={isRecording ? `Recording... 0:${recordingTime.toString().padStart(2, '0')}` : input} onChange={handleInputChange} placeholder="type a message... (*bold* _italic_)" disabled={isRecording} className={`w-full p-2 sm:p-3 retro-border retro-bg-window focus:outline-none font-bold placeholder:font-normal text-sm sm:text-base ${isRecording ? 'text-red-500 animate-pulse bg-red-50' : ''}`} />
                </div>
                ...
                ) : ( <RetroButton type="submit" variant="primary" className="p-2 sm:p-3 flex-shrink-0"><Send size={18} /></RetroButton> )}
              </form>
            </div>
          </div>
          {showDetails && (
