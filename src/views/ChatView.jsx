...
export function ChatView({ onClose, profile, partnerNickname, sfx, chatHistory, setChatHistory, userId, partnerId }) {
  ...
  useEffect(() => { 
    if (!activeOptions && searchQuery === '' && !viewingImageId) {
      messagesEndRef.current?.scrollIntoView({ behavior: "smooth" }); 
    }
  }, [chatHistory, activeOptions, searchQuery, viewingImageId, showDetails]);
  ...
  const confirmVoiceNote = () => {
    ...
    setChatHistory(prev => [...prev, { 
      ...
      audioUrl: voiceBase64, // Synced as Base64 string
  ...
  const handleFileChange = (e) => { 
    ...
    if (file) { 
      ...
      reader.onloadend = () => {
        playAudio('send', sfx);
        setChatHistory(prev => [...prev, { id: Date.now(), sender: userId, senderName: profile?.name, type: 'image', url: reader.result, text: file.name, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }), replyTo: replyingTo, status: 'sent' }]); 
        setReplyingTo(null);
      };
        ...
        <div className="flex flex-1 h-full overflow-hidden relative">
          ...
          <div className={`flex flex-col h-full transition-all duration-300 ${showDetails ? 'hidden md:flex md:w-2/3 border-r-2 retro-border' : 'w-full'}`}>
            <div className="flex-1 overflow-y-auto p-4 flex flex-col retro-bg-window relative">
              ...
              {filteredMessages.map((msg, index) => {
                ...
                // Don't show technical call invites in the main chat feed if they are system-only
                          ...
                          <>
                            ...
                            {msg.type === 'call_invite' && (
                              <div className="flex items-center gap-2">
                                ...
                                <span className="font-bold text-sm uppercase">Call {msg.status}</span>
                              </div>
                            )}
            ...
            <div className="flex flex-col retro-bg-accent retro-border-t">
              {voicePreview !== null && (
                <div className="p-3 bg-blue-50 border-b border-dashed border-[var(--border)] flex items-center justify-between gap-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => { playAudio('click', sfx); discardVoiceNote(); startRecording(); }} className="px-3 py-1 retro-border bg-blue-200 text-blue-800 text-xs font-bold hover:bg-blue-300 rounded">Re-record</button>
              ...
              {replyingTo && (<div className="p-2 bg-white/50 border-b border-dashed border-[var(--border)] flex justify-between items-center text-sm"><div><span className="font-bold mr-2 text-[var(--primary)]"><Reply size={14} className="inline mr-1"/>Replying to {replyingTo.sender === userId ? profile.name || 'You' : 'Partner'}:</span><span className="opacity-70 truncate max-w-[200px] inline-block align-bottom">{replyingTo.text || 'Attachment/Voice'}</span></div><button onClick={() => {playAudio('click', sfx); setReplyingTo(null)}} className="p-1 hover:bg-black/10 rounded-full"><X size={14}/></button></div>)}
          ...
          {showDetails && (
            <div className="flex flex-col w-full md:w-1/3 retro-bg-main overflow-y-auto relative border-l-2 retro-border">
              ...
              <div className="p-2 flex gap-1 bg-[var(--border)] shrink-0">
                <button onClick={() => setActiveSidebarTab('media')} className={`flex-1 py-1 text-[10px] font-bold uppercase ${activeSidebarTab === 'media' ? 'bg-white' : 'opacity-50 text-white'}`}>Media</button>
                <button onClick={() => setActiveSidebarTab('calls')} className={`flex-1 py-1 text-[10px] font-bold uppercase ${activeSidebarTab === 'calls' ? 'bg-white' : 'opacity-50 text-white'}`}>Calls</button>
                <button onClick={() => setActiveSidebarTab('search')} className={`flex-1 py-1 text-[10px] font-bold uppercase ${activeSidebarTab === 'search' ? 'bg-white' : 'opacity-50 text-white'}`}>Search</button>
              </div>
              ...
              <div className="p-4 flex flex-col gap-6">
                ...
                {activeSidebarTab === 'media' && (
                  <>
                    <div>
                      <h3 className="font-bold border-b-2 border-[var(--border)] pb-2 mb-4 flex items-center gap-2"><Pin size={16}/> Pinned</h3>
                    ...
                    <div>
                      <h3 className="font-bold border-b-2 border-[var(--border)] pb-2 mb-4 flex items-center gap-2"><ImageIcon size={16}/> Media Grid</h3>
                ...
                {activeSidebarTab === 'calls' && (
                  <div>
                    <h3 className="font-bold border-b-2 border-[var(--border)] pb-2 mb-4 flex items-center gap-2"><History size={16}/> Call History</h3>
                    <div className="flex flex-col gap-2 overflow-y-auto pr-1">
                      {callHistory.length === 0 ? (<p className="text-sm opacity-50">No call history.</p>) : (callHistory.reverse().map(m => (
                        <div key={m.id} className="bg-[var(--bg-window)] p-3 text-xs retro-border flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            {m.callType === 'video' ? <Video size={14} className="text-[var(--secondary)]"/> : <Phone size={14} className="text-[var(--primary)]"/>}
                            <div>
                               <p className="font-bold">{m.sender === userId ? 'Outgoing' : 'Incoming'}</p>
                               <p className="opacity-50">{m.time}</p>
                            </div>
                          </div>
                          <span className={`font-bold uppercase text-[9px] px-1.5 py-0.5 retro-border ${m.status === 'accepted' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{m.status}</span>
                        </div>
                      )))}
                    </div>
                  </div>
