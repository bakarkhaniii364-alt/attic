...
export function ScrapbookApp({ onClose, images, sfx }) {
  const [page, setPage] = useState(1); const visibleImages = images.slice(0, page * 6);
  return (
    <RetroWindow title="memories.exe" onClose={onClose} className="w-full max-w-4xl h-[calc(100dvh-4rem)] max-h-[800px] flex flex-col" noPadding>
      <div className="p-4 bg-[var(--bg-main)] border-b-2 retro-border flex items-center justify-between"><h2 className="font-bold text-xl"><ImageIcon className="inline mr-2"/> Photo Grid</h2><RetroButton className="px-4 py-2 text-xs flex items-center gap-2"><Camera size={14}/> Upload</RetroButton></div>
      <div className="flex-1 overflow-y-auto p-4 flex flex-col bg-[var(--bg-window)]">
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">{visibleImages.map((url, i) => <img key={i} src={url} alt="memory" loading="lazy" className="aspect-square object-cover retro-border retro-shadow-dark w-full" />)}</div>
        {visibleImages.length < images.length && <RetroButton onClick={() => {playAudio('click', sfx); setPage(p=>p+1)}} variant="white" className="mx-auto py-2 px-6">Load More</RetroButton>}
      </div>
    </RetroWindow>
  );
}
